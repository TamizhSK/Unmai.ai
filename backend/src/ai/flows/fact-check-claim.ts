...
function cleanAndParseJson(responseText: string): Record<string, unknown> | null {
  ...
  try {
    ...
    console.log('[WARN] First parse attempt failed:', (firstError as Error).message);
    
    // Extract error position for debugging
    const posMatch = (firstError as Error).message.match(/position (\d+)/i);
    if (posMatch) {
    ...
    try {
      ...
      console.log('[ERROR] Aggressive cleaning failed:', (secondError as Error).message);
      ...
      try {
        ...
        console.log('[ERROR] Manual extraction failed:', (thirdError as Error).message);
      }
    }
  }
 ...
 */
function extractKeyValuePairs(jsonStr: string): Record<string, unknown> | null {
 ...
 */
function extractFromText(responseText: string): Record<string, unknown> {
