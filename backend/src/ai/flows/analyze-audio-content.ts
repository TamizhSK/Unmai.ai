...
async function transcribeAudio(audioData: string, mimeType?: string) {
  ...
  // Skip Speech API in development mode
  if (isDevelopment) {
    console.log('[INFO] Development mode - skipping Speech-to-Text API');
    return '[Audio transcription not available in development mode - Speech API requires GCP authentication]';
  }
