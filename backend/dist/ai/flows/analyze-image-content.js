...
function buildImagePart(imageData, mimeType) {
...
function likelihoodToString(likelihood) {
...
function toVisionImagePayload(imageData) {
    if (!imageData) {
        return {};
    }
    if (imageData.startsWith('data:')) {
...
async function geminiImageUnderstanding(imageData, mimeType) {
    ...
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            return {
                ...
                detectedObjects: Array.isArray(parsed.detectedObjects) ? parsed.detectedObjects.map((v) => String(v)) : [],
                recognizedText: Array.isArray(parsed.recognizedText) ? parsed.recognizedText.map((v) => String(v)) : [],
                locations: Array.isArray(parsed.locations) ? parsed.locations.map((v) => String(v)) : [],
                authenticityScore: Number.isFinite(parsed.authenticityScore) ? Math.max(0, Math.min(100, Number(parsed.authenticityScore))) : 50,
                potentialIssues: Array.isArray(parsed.potentialIssues) ? parsed.potentialIssues.map((v) => String(v)) : [],
                recommendedFactChecks: Array.isArray(parsed.recommendedFactChecks) ? parsed.recommendedFactChecks.map((v) => String(v)) : [],
            };
        }
    }
    catch (error) {
...
async function buildGeminiGuidedSearchQueries(understanding, ocrText) {
    try {
...
function buildReverseImageQueries(understanding, ocrText) {
    const queries = [];
    const add = (value) => {
...
async function reverseImageGrounding(queries, searchEngineId) {
    const results = [];
...
async function analyzeImageTextWithVertex(ocrText) {
...
- Return JSON only (no prose).`;
    try {
        ...
        if (parsed && typeof parsed === 'object') {
            ...
            const normalizeVerdict = (value) => {
                ...
                return upper === 'VERIFIED' || upper === 'DISPUTED' ? upper : 'UNVERIFIED';
            };
            const rawClaims = rawClaimsInput
                ...
                .filter((entry) => entry.claim.length > 10)
                .slice(0, 5);
            const riskIndicators = Array.isArray(parsed.riskIndicators)
                ? parsed.riskIndicators.map((indicator) => String(indicator || '').trim()).filter(Boolean)
                : [];
            return {
                summary,
                rawClaims,
    ...
    const fallbackClaims = trimmed
        ...
        .slice(0, 3)
        .map((sentence) => ({ claim: sentence, verdict: 'UNVERIFIED', confidence: 0.4 }));
    return {
...
async function extractImageMetadata(imageData) {
    ...
    try {
        ...
        const possibleSources = (result.webDetection?.pagesWithMatchingImages || [])
...
function deriveReverseImageQueries({ metadata, ocrText, vertexInsights, }) {
...
async function analyzeImageContentAndFactCheck(imageData, ocrText) {
...
async function getVertexImageInsights(imageData, mimeType) {
    try {
...
- Respond with JSON only.`;
        ...
        if (parsed && typeof parsed === 'object') {
            const normalizeArray = (value) => Array.isArray(value)
                ? value.map((entry) => String(entry || '').trim()).filter(Boolean).slice(0, 10)
                : [];
            const clamp = (num) => {
...
async function detectImageDeepfake(imageData) {
...
Base your assessment only on the provided pixels. Mention if OCR or metadata was unavailable.`;
    ...
    const attemptVisionAnalysis = async () => {
        try {
            ...
            const { isManipulated = false, confidence: rawConfidence = 0.5, authenticitySummary = '', suspectedTechniques, visualIndicators, protectionAdvice, metadataNotes = '', } = parsed;
...
function calculateScores(contentAnalysis, manipulationAnalysis) {
    ...
    const contentAuthenticityScore = Math.round(Math.max(0, baseAuthenticity + confidenceBoost - disputePenalty - watermarkPenalty - aiPenalty));
...
export async function analyzeImageContent(input, options) {
    try {
        ...
        const combinedManipulationConfidence = Math.max(deepfakeInfo.manipulationConfidence, vertexInsights?.aiConfidence ?? 0);
        ...
        const combinedWatermarkConfidence = Math.max(visionMetadata.watermarkFindings?.confidence ?? 0, vertexInsights?.watermarkConfidence ?? 0);
        ...
        const combinedSuspectedAIGenerated = Boolean(vertexInsights?.suspectedAIGenerated || visionMetadata.suspectedAIGenerated);
        ...
        const searchResults = await Promise.allSettled(reverseSearchQueries.slice(0, 3).map((query) => performWebAnalysis({
            query,
            contentType: 'text',
            mediaType: 'image',
        ...
        const safeSearchConcern = ['adult', 'violence', 'racy'].some((field) => {
            const value = safeSearchFlags[field];
        ...
        const textRiskConcern = Boolean(vertexTextInsights?.riskIndicators?.some((indicator) => /fake|misinfo|misleading|scam|warning|propaganda|deceptive/i.test(indicator)));
        let analysisLabel = 'YELLOW';
        if (combinedIsManipulated && combinedManipulationConfidence >= 0.7) {
            analysisLabel = 'RED';
        }
        else if (safeSearchConcern ||
            watermarkConcern ||
            vertexWatermarkConcern ||
            vertexAIGeneratedConcern ||
            textRiskConcern ||
            suspectedAIGenerated ||
            contentAnalysis.factualClaims.some((c) => c.verdict === 'DISPUTED')) {
            analysisLabel = 'ORANGE';
        }
        else if (!combinedIsManipulated &&
            !watermarkConcern &&
            !vertexWatermarkConcern &&
            !suspectedAIGenerated &&
            contentAnalysis.factualClaims.length > 0 &&
            contentAnalysis.factualClaims.every((c) => c.verdict === 'VERIFIED')) {
            analysisLabel = 'GREEN';
        }
...
//# sourceMappingURL=analyze-image-content.js.map